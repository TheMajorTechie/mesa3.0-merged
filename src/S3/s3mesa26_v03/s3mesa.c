...
#ifdef DEBUG_PRIMITIVES
	fprintf(stderr, "2: %f, %f, %f RGB %d %d %d UVW %f %f %7.8f\n", v[0]->X, v[0]->Y, v[0]->Z, v[0]->R, v[0]->G, v[0]->B, v[0]->U, v[0]->V, v[0]->W);
	fprintf(stderr, " : %f, %f, %f RGB %d %d %d UVW %f %f %7.8f\n", v[1]->X, v[1]->Y, v[1]->Z, v[1]->R, v[1]->G, v[1]->B, v[1]->U, v[1]->V, v[1]->W);
	fprintf(stderr, " : %f, %f, %f RGB %d %d %d UVW %f %f %7.8f\n", v[2]->X, v[2]->Y, v[2]->Z, v[2]->R, v[2]->G, v[2]->B, v[2]->U, v[2]->V, v[2]->W);
#endif
...
#ifdef DEBUG_PRIMITIVES
	fprintf(stderr, "F: %f, %f, %f RGB %d %d %d UVW %f %f %7.8f\n", v[0]->X, v[0]->Y, v[0]->Z, v[0]->R, v[0]->G, v[0]->B, v[0]->U, v[0]->V, v[0]->W);
	fprintf(stderr, " : %f, %f, %f RGB %d %d %d UVW %f %f %7.8f\n", v[1]->X, v[1]->Y, v[1]->Z, v[1]->R, v[1]->G, v[1]->B, v[1]->U, v[1]->V, v[1]->W);
	fprintf(stderr, " : %f, %f, %f RGB %d %d %d UVW %f %f %7.8f\n", v[2]->X, v[2]->Y, v[2]->Z, v[2]->R, v[2]->G, v[2]->B, v[2]->U, v[2]->V, v[2]->W);
#endif
...
//#ifdef DEBUG_TEXTURES
			...
			gl_error (s3ctx->gl_ctx, GL_OUT_OF_MEMORY,
				"S3Mesa: Couldn't allocate memory for textures in video memory");
			MessageBox(GetDesktopWindow(),"S3Mesa: Couldn't allocate memory for textures in video memory",
								"Out of memory - switching to software Opengl",MB_OK|MB_SYSTEMMODAL);
...
static int SelectTexture(s3MesaContext s3ctx, GLuint texObject)
{
	...
	if (!info->data || (info->size > (512*512*2))) {
		fprintf(stderr, "SelectTexture: idiot check failed in texture %d size=%d\n", texObject,info->size);
		return(0);
	}
 ...
 * Texture parameter
 */
static void texparam(GLcontext *ctx, GLenum target, /*GLuint texObject*/struct gl_texture_object* stTexObj,
					 GLenum pname, const GLfloat *params)
{
	s3MesaContext s3ctx=(s3MesaContext)ctx->DriverCtx;
	GLenum param=(GLenum)(GLint)params[0];
	GLuint texObject=stTexObj->Name;
 ...
 * Texture delete
 */
static void texdel(GLcontext *ctx, struct gl_texture_object * sttexObject)
{
	s3MesaContext s3ctx=(s3MesaContext)ctx->DriverCtx;
	GLuint texObject=sttexObject->Name;
	texinfo *info = &s3ctx->ti[texObject];
...
static void texbind(GLcontext *ctx, GLenum target, struct gl_texture_object * sttexObject)
{
	s3MesaContext s3ctx=(s3MesaContext)ctx->DriverCtx;
	GLuint texObject = sttexObject->Name;
	texinfo *info = &s3ctx->ti[texObject];
	...
	s3ctx->currenttex = texObject;
	if(info->data)//Raja - this'll avoid all those idiot texture messages
	UpdateTextureState(ctx);
}
...
static void teximg(GLcontext *ctx, GLenum target,
                   /*GLuint texObject*/struct gl_texture_object* sttexObject, GLint level, GLint internalFormat,
...
//		texdel(ctx, texObject);
	...
	if((level > 0) && !ti[texObject].data) //This cannot happen
	{
		fprintf(stderr, "Cannot have higher than level 0 allocated before level 0\n");
		return;
	}
	
	/* For first level only, allocate space for all levels and clear the base data */

	if (level == 0 && !ti[texObject].data) {
		...
		ti[texObject].size = sizeof(word)*image->Width*image->Height*22/16;
		...
		int startaddr=0;
		int i;
		int w=ti[texObject].width,h=ti[texObject].height;
		for(i=0;i<level;i++)
		{
			startaddr+=w*h*2;
			w=w/2;h=h/2;
		...
		converted_texture = (word *) ((char *) ti[texObject].data + startaddr);
		if(level > 0)
	...
	if (!TexBuildS3Format(converted_texture, image, internalFormat, &transparent, &s3format)) {
		...
		if(ti[texObject].data) free(ti[texObject].data);
		memset(&ti[texObject],0,sizeof(texinfo));
		return;
	}
...
//	ti[texObject].mipmaplevels++;
	if(level>ti[texObject].mipmaplevels)
		ti[texObject].mipmaplevels=level;
...
static void SetNearFar( GLcontext *ctx, GLfloat n, GLfloat f)
{
	...
	if(s3ctx) {
		s3ctx->hither	= fabs(n);
		s3ctx->yon		= fabs(f);
		s3ctx->wscale	= (s3ctx->yon != 0.0f) ?65535.0f / s3ctx->yon:65535.0f/100.0f;
#ifdef DEBUG_PRIMITIVES
        fprintf(stderr,"Setting near & far %f %f wscale=%f\n",n,f,s3ctx->wscale);
#endif
					...
					GLubyte blue[], GLubyte alpha[] )
{
	int i, offset;
	dword stride;
	word colour, *screen_ptr;//this will not work for 24-bit 
	LPDIRECTDRAWSURFACE *surface;
	s3MesaContext s3ctx=(s3MesaContext)ctx->DriverCtx;
	...
	offset = y*stride + x;
		...
		/* draw all pixels */
		for (i=0; i<n; i++, x++) {
			colour=screen_ptr[i+offset] ;
			red[i]=(colour&0x7c00)>>7;
			green[i]=(colour&0x03e0)>>2;
			blue[i]=(colour&0x001f)<<3;
			alpha[i]=0;
...
void s3MesaResizeWindow(s3MesaContext s3ctx, int w, int h)
{
	...
	if (reallocate_back) {
		if (DDSetupBackSurface(&s3ctx->back_surface, w, h) != DD_OK) {
			...
			s3ctx->renderdisabled |= 1;
			BuildDispatchTable(s3ctx->gl_ctx);
			MessageBox(GetDesktopWindow(),"S3Mesa: Couldn't allocate memory for back buffer in video memory",
								"Out of memory - switching to software Opengl",MB_OK|MB_SYSTEMMODAL);
			gl_error (s3ctx->gl_ctx, GL_OUT_OF_MEMORY,
				"S3Mesa: Couldn't allocate memory for back buffer in video memory");
	...
	if (reallocate_z) {
		if (DDSetupZSurface(&s3ctx->z_surface, w, h) != DD_OK) {
			...
			MessageBox(GetDesktopWindow(),"S3Mesa: Couldn't allocate memory for Z buffer in video memory",
								"Out of memory - switching to software Opengl",MB_OK|MB_SYSTEMMODAL);
			gl_error (s3ctx->gl_ctx, GL_OUT_OF_MEMORY,
				"S3Mesa: Couldn't allocate memory for Z buffer in video memory");
			return;
		}
	}
